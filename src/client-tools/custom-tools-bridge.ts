...
const CAPTURE_SENTINEL =
  "[routed-to-client] This tool runs on the Hermes control plane. The call has " +
...
/**
 * Bridges the two tool planes. The caller's (Hermes) client tools are registered
 ...
 *    The executor (Hermes) still holds the real schema and validates on
 *    execution, so the model only needs name + arg names to emit a correct call.
 *
