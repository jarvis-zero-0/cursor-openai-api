 ...
 * names. The caller (e.g. Hermes) already holds the real schemas. So rarely-used
...
/**
 * High-frequency tools that stay resident (full schema) in `tiered` mode when
 * `CURSOR_TOOL_RESIDENT` is not set. Tuned for the Hermes orchestrator path
 * (router-only tools); other deployments can override via env.
 */
 ...
 * `tiered` with `DEFAULT_RESIDENT_TOOLS` when unset.
 */
