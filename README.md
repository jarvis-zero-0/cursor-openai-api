...
- Optionally pass `metadata.cursor_skill_note` (or the `X-Cursor-Skill-Note` header) to steer the leaf at a specific skill. On a fresh native agent the proxy appends it to the directive under a `SKILL ROUTING (from orchestrator):` block. The directive also carries a static `SKILLS` block telling the leaf its Cursor skills are auto-loaded and that Hermes control-plane tools (`skill_view` / `skills_list` / `skill_manage`) are not its to call.
