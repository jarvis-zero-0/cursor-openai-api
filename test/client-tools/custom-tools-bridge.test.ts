...
describe("buildClientToolCustomTools", () => {
  test("maps each spec to an SDK custom tool keyed by name", () => {
    ...
    expect(tools.session_search.description).toBe("Search past sessions");
    expect(tools.session_search.inputSchema).toEqual({
      type: "object",
      properties: { query: { type: "string" } },
    });
    // Tools without description/parameters omit those fields.
    expect(tools.memory.description).toBeUndefined();
    expect(tools.memory.inputSchema).toBeUndefined();
    expect(typeof tools.memory.execute).toBe("function");
  });
...
describe("ClientToolCaptureSink cancellation", () => {
  test("a captured call triggers the bound cancel exactly once", async () => {
    ...
    sink.record("memory", { action: "add" });
    sink.record("session_search", { query: "x" });
